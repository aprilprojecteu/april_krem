...
class PlanVisualization:
    def __init__(self) -> None:
        ...
        self.plan_pub = rospy.Publisher(
            "/dot_graph_visualization/dot_graph", String, queue_size=1
        )
    ...
    def set_actions(self, actions: Sequence[object]) -> None:
        ...
        self.nodes = [
            Node(str(action), style="filled", fillcolor="white") for action in actions
        ]
        for node in self.nodes:
            self.graph.add_node(node)
        self.edges = [
            Edge(node, self.nodes[number])
            for number, node in enumerate(self.nodes[:-1], start=1)
        ]
